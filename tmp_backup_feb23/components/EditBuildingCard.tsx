...
import { X, Trash2, LogIn, Hash, Box, Layers, GripHorizontal, User, RotateCw, BoxSelect, Maximize2, Move, Eye, EyeOff, Lock, Unlock, Upload, History, Loader2, Plus } from 'lucide-react';
...
const EditBuildingCard: React.FC<EditBuildingCardProps> = ({ building, buildingDefs, onUpdate, onEnter, onClose, mode, onDelete, statuses, owners, viewLevel }) => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIsUploading(true);
    try {
      const url = await convertPdfToImage(file, 1.0);
      ...
      const plans = [...(building.floorPlans || []), newPlan];
      ...
      alert("Failed to process PDF. Please try a valid PDF file.");
    } finally {
  ...
  const activePlan = (building.floorPlans || []).find(p => p.id === building.activeFloorPlanId);

  const updateActivePlan = (updates: Partial<FloorPlan>) => {
    if (!activePlan) return;
    const newPlans = (building.floorPlans || []).map(p => p.id === activePlan.id ? { ...p, ...updates } : p);
    onUpdate(building.id, { floorPlans: newPlans });
  };
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex gap-1">
              ...
              <button
                ...
                className="flex items-center gap-1 px-2 py-1 bg-blue-50 text-blue-600 rounded-lg text-[8px] font-black uppercase hover:bg-blue-100 disabled:opacity-50"
              >
          ...
          {(building.floorPlans || []).length > 0 && (
            <div className="space-y-3 bg-slate-50 p-3 rounded-xl border border-slate-200">
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <select
                  value={building.activeFloorPlanId || ''}
                  onChange={e => onUpdate(building.id, { activeFloorPlanId: e.target.value })}
                  className="flex-1 text-[9px] font-bold bg-white border border-slate-300 rounded-lg py-1 px-2 outline-none text-slate-900"
                  disabled={isReadOnly}
                >
                  {(building.floorPlans || []).slice().reverse().map(p => (
                    <option key={p.id} value={p.id}>
                      {p.name} ({new Date(p.uploadDate).toLocaleDateString()})
                    </option>
                  ))}
                </select>
              </div>
                ...
                <div className="space-y-2 animate-in slide-in-from-top-2">
                  ...
                  <div className="flex items-center justify-between bg-white border border-slate-300 rounded-lg p-1.5">
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        onClick={() => updateActivePlan({ locked: !activePlan.locked })}
                        className={`p-1.5 rounded-md transition-colors ${activePlan.locked ? 'bg-amber-100 text-amber-600' : 'text-slate-400 hover:text-slate-600'}`}
                        title={activePlan.locked ? "Unlock Position" : "Lock Position"}
                      >
                        {activePlan.locked ? <Lock size={14} /> : <Unlock size={14} />}
                      </button>
                    </div>
                    <button
                      onClick={() => {
                        if (confirm('Archive this floor plan?')) {
                          const newPlans = building.floorPlans!.filter(p => p.id !== activePlan.id);
                          onUpdate(building.id, { floorPlans: newPlans, activeFloorPlanId: newPlans[0]?.id || null });
                        }
                      }}
                  ...
                  {!activePlan.locked && (
                    <>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <label className="text-[8px] font-black text-slate-400 uppercase block mb-1">Opacity</label>
                          <input
                            type="range"
                            min="0.1" max="1" step="0.1"
                            value={activePlan.opacity}
                            onChange={e => updateActivePlan({ opacity: parseFloat(e.target.value) })}
                            className="w-full accent-blue-600 h-1 bg-slate-200 rounded-lg appearance-none cursor-pointer"
                          />
                        </div>
                        <div>
                          <label className="text-[8px] font-black text-slate-400 uppercase block mb-1">Scale</label>
                          <input
                            type="number" step="0.01"
                            value={activePlan.scale}
                            onChange={e => updateActivePlan({ scale: parseFloat(e.target.value) })}
                            className="w-full text-[9px] font-bold bg-white border border-slate-300 rounded px-1 py-0.5 text-center text-slate-900"
                          />
                        </div>
                      </div>
                      ...
                      <div className="bg-white border border-slate-300 rounded-lg p-2">
                        <label className="text-[8px] font-black text-slate-400 uppercase block mb-1.5 text-center">Position & Rotation</label>
                        <div className="grid grid-cols-3 gap-1 mb-1">
                          <div className="flex items-center gap-1">
                            <span className="text-[8px] font-mono text-slate-400">X</span>
                            <input type="number" step="0.5" value={activePlan.x} onChange={e => updateActivePlan({ x: parseFloat(e.target.value) })} className="w-full text-[9px] bg-slate-50 rounded px-1 text-center font-mono text-slate-900" />
                          </div>
                          <div className="flex items-center gap-1">
                            <span className="text-[8px] font-mono text-slate-400">Z</span>
                            <input type="number" step="0.5" value={activePlan.z} onChange={e => updateActivePlan({ z: parseFloat(e.target.value) })} className="w-full text-[9px] bg-slate-50 rounded px-1 text-center font-mono text-slate-900" />
                          </div>
                          <div className="flex items-center gap-1">
                            <RotateCw size={8} className="text-slate-400" />
                            <input type="number" step="90" value={Math.round(activePlan.rotation * (180 / Math.PI))} onChange={e => updateActivePlan({ rotation: parseFloat(e.target.value || '0') * (Math.PI / 180) })} className="w-full text-[9px] bg-slate-50 rounded px-1 text-center font-mono text-slate-900" />
                          </div>
                        </div>
                      </div>
                    </>
