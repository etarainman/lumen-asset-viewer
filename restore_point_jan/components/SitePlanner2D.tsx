...
const BuildingShell: React.FC<{
  ...
  building, buildingDefs, isSelected, isBeingDragged, isGhost = false, appMode, colorMode, colorCodingEnabled, onPointerDown, onRotateClick, statuses, owners, zoom, showLabels
}) => {
    ...
    // Updated: Refined labels for feet-scale (1.8px), text-only with no container background
    const labelClasses = "text-[1.8px] text-white tracking-tight leading-tight drop-shadow-[0_0.5px_0.5px_rgba(0,0,0,1)] whitespace-nowrap text-center font-black";
...
const SitePlanner2D: React.FC<SitePlanner2DProps> = ({
  buildings, buildingDefs, selectedBuildingId, appMode, colorMode, colorCodingEnabled, onSelectBuilding, onUpdateBuilding, onAddBuilding, statuses, owners, isDirty, isSaving, saveSuccess, onSave, onDeleteBuilding, onEnterBuilding, showLabels
}) => {
  ...
  return (
    <div className="flex h-full w-full bg-[#020617] overflow-hidden font-sans relative">
      ...
      }} style={{ backgroundImage: `linear-gradient(rgba(59, 130, 246, 0.03) 1px, transparent 1px), linear-gradient(90deg, rgba(59, 130, 246, 0.03) 1px, transparent 1px)`, backgroundSize: `${5 * zoom}px ${5 * zoom}px`, backgroundPosition: `${offset.x}px ${offset.y}px` }}>
