...
// --- RECOVERY DATA FROM constants.ts (Verified full set) ---
...
async function recoverAdminFoundation() {
    console.log('--- CRITICAL ADMIN FOUNDATION RECOVERY ---');

    // 1. Fetch current workspace (to preserve site/building/rack data)
    ...
    // 2. Re-inject ALL FOUNDATIONAL REGISTRIES
    console.log(`Current counts: Buildings=${data.buildingDefs?.length}, Racks=${data.rackDefs?.length}, Equip=${data.equipmentDefs?.length}`);
    ...
    console.log('Registries successfully re-populated from constants.ts baseline.');
    ...
    if (updateError) {
        ...
        console.log('--- ADMIN FOUNDATION RESTORED ---');
