...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const hydrate = async () => {
      ...
      if (initialData) {
        ...
        setRackDefs(initialData.rackDefs || INITIAL_RACK_DEFS);
        setEquipmentDefs(initialData.equipmentDefs || INITIAL_EQUIPMENT_DEFS);
        ...
        if (initialData.buildings) setBuildings(initialData.buildings);
  ...
  const handleAddRack = useCallback(() => {
    ...
    const newRack: Rack = {
        ...
        x: activeBuilding.racks.length * 80 - 200, 
        y: 0,
            ...
            <div className="pointer-events-auto">
              {(() => {
                ...
                return (
                  ...
                  onUpdate={(id, up) => { const newRacks = activeBuilding.racks.map(r => r.id === id ? { ...r, ...up } : r); handleUpdateBuilding(activeBuilding.id, { racks: newRacks }); }} onMove={(dir) => { 
                    const step = 5; 
                    let { x, y } = { x: rack.x, y: rack.y }; 
                    if (dir === 'F') y += step; 
                    if (dir === 'B') y -= step; 
                    if (dir === 'L') x += step; 
                    if (dir === 'R') x -= step; 
                    const newRacks = activeBuilding.racks.map(r => r.id === selectedRackId ? { ...r, x, y } : r); 
                    handleUpdateBuilding(activeBuilding.id, { racks: newRacks }); 
                  }} onClose={() => setSelectedRackId(null)} mode={appMode} onDelete={(id) => { const newRacks = activeBuilding.racks.filter(r => r.id !== id); handleUpdateBuilding(activeBuilding.id, { racks: newRacks }); setSelectedRackId(null); }} 
