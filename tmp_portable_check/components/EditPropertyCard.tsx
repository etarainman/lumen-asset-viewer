...
const EditPropertyCard: React.FC<EditPropertyCardProps> = ({ 
  ...
  return (
    <>
    <div className="absolute w-[280px] bg-white shadow-2xl rounded-3xl border border-slate-200 z-[120] animate-in fade-in zoom-in duration-300 flex flex-col overflow-hidden" style={{ left: `${position.x}px`, top: `${position.y}px` }}>
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="space-y-3">
          <div>
            ...
            <select value={item.definitionId} onChange={e => onUpdate(item.id, { definitionId: e.target.value })} disabled={isReadOnly} className="w-full text-[10px] font-bold border-b border-slate-100 pb-1 outline-none bg-transparent uppercase truncate text-slate-950">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select value={item.vendorId} onChange={e => onUpdate(item.id, { vendorId: e.target.value })} disabled={isReadOnly} className="w-full text-[9px] font-black border-b border-slate-100 pb-1 outline-none bg-transparent uppercase text-slate-950">
                {vendors.map(v => <option key={v.id} value={v.id}>{v.name}</option>)}
              </select>
            </div>
            <div>
              <label className="text-[8px] font-black text-slate-500 uppercase mb-1 block tracking-widest">Stakeholder</label>
              <select value={item.ownerId} onChange={e => onUpdate(item.id, { ownerId: e.target.value })} disabled={isReadOnly} className="w-full text-[9px] font-black border-b border-slate-100 pb-1 outline-none bg-transparent uppercase text-slate-950">
        ...
        <div>
            ...
            <select value={item.status} onChange={e => onUpdate(item.id, { status: e.target.value as Status4D })} disabled={isReadOnly || mode === 'EDIT'} className="w-full text-[9px] font-black border border-slate-200 rounded-lg p-1.5 outline-none bg-slate-50 text-slate-950 appearance-none uppercase">
