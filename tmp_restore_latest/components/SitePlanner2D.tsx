...
const AlertBadge: React.FC<{ alerts: any[], buildingId: string, w: number, h: number }> = ({ alerts, buildingId, w, h }) => {
    try {
        ...
        return (
            <g transform={`translate(${w}, 0)`}>
                {/* Outer shadow/glow effect (optional, removed for sharpness) */}
                <circle
                    r="2.5"
                    fill="white"
                    stroke="none"
...
const BuildingShell: React.FC<{
    ...
    building, buildingDefs, isSelected, isBeingDragged, isGhost = false, appMode, colorMode, colorCodingEnabled, onPointerDown, onRotateClick, statuses, owners, zoom, showLabels, alerts
        ...
        const labelClasses = "text-[1.2px] text-white tracking-tight leading-tight drop-shadow-[0_0.2px_0.2px_rgba(0,0,0,1)] whitespace-nowrap text-center font-black";
        ...
        return (
            ...
            >
                ...
                >
                    ...
                    {/* Move AlertBadge inside SVG to ensure non-scaling border */}
                    <AlertBadge alerts={alerts} buildingId={building.id} w={w} h={0} />
                </svg>
