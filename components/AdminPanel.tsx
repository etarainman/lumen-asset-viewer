...
import { Database, Cloud, History, X, Plus, Trash2, Box, Save, Layout, Layers, BoxSelect, Server, HardDrive, FileText, Image as ImageIcon, Upload, Search, ArrowUpDown, Filter, Power, Edit3, MoreHorizontal, Check } from 'lucide-react';
...
const AdminPanel: React.FC<AdminPanelProps> = ({
  buildingDefs, setBuildingDefs, rackDefs, setRackDefs, equipmentDefs, setEquipmentDefs, vendors, setVendors, sites, setSites, cloudConfig, setCloudConfig, onClose, history, isDirty, onSave, isSaving, saveSuccess, saveError, onExportData, onImportData, statuses, setStatuses, owners, setOwners
}) => {
        ...
        <div className="flex-1 bg-white overflow-y-auto p-12">
          ...
          {activeTab === 'RACKS' && bufferRack ? (
            ...
            <div className="max-w-4xl space-y-10 animate-in fade-in slide-in-from-right-10 duration-500">
              <div className="flex justify-between items-start">
                <div>
                  <h2 className="text-3xl font-black text-slate-950 uppercase tracking-tight">Cloud Synchronization</h2>
                  <p className="text-xs text-slate-500 mt-2 font-bold uppercase tracking-widest">Supabase Engine & Data Residency</p>
                </div>
                <div className="flex gap-2">
                  <button
                    onClick={onSave}
                    disabled={isSaving}
                    className={`text-[10px] font-black uppercase px-6 py-3 rounded-xl shadow-lg transition-all disabled:opacity-50 flex items-center gap-2 ${saveSuccess ? 'bg-emerald-500 text-white' : 'bg-emerald-600 text-white hover:bg-emerald-500 shadow-emerald-500/20'}`}
                  >
                    {isSaving ? (
                      <>
              ...
              <div className="grid grid-cols-2 gap-10">
                <div className="space-y-6">
                  <div className="p-8 bg-slate-50 rounded-[32px] border border-slate-200 shadow-inner space-y-6">
                    <div>
                      <label className={labelClasses}>Supabase API URL</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className={labelClasses}>Anon (Public) Key</label>
                      <textarea
                    ...
                    <div>
                      <label className={labelClasses}>Cloud Workspace ID</label>
                      <input
                        type="text"
                        value={cloudConfig.workspaceId}
                        onChange={e => setCloudConfig({ ...cloudConfig, workspaceId: e.target.value })}
                        className={inputClasses}
                      />
                    </div>
                  </div>
                ...
                <div className="space-y-6">
                  <div className="bg-slate-900 rounded-[40px] p-8 flex flex-col items-center justify-center shadow-2xl relative overflow-hidden">
                    <div className={`absolute inset-0 transition-opacity duration-1000 ${saveSuccess ? 'bg-emerald-500/5 opacity-100' : saveError ? 'bg-red-500/5 opacity-100' : 'opacity-0'}`} />
                    <Database size={80} className={`mb-4 transition-all duration-500 ${saveSuccess ? 'text-emerald-500 scale-110' : saveError ? 'text-red-500' : 'text-slate-700'}`} />
                    <div className="text-center space-y-2 relative z-10">
                      <span className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500 block">Connection Status</span>
                      <div className="flex items-center justify-center gap-2">
                        <div className={`w-2 h-2 rounded-full ${saveSuccess ? 'bg-emerald-500 animate-pulse' : saveError ? 'bg-red-500' : 'bg-slate-700'}`} />
                        <span className={`text-[11px] font-black uppercase ${saveSuccess ? 'text-emerald-500' : saveError ? 'text-red-500' : 'text-slate-600'}`}>
                          {saveSuccess ? 'Synchronized' : saveError ? 'Connection Failed' : 'IDLE / Offline'}
                        </span>
                      </div>
                    </div>
                  </div>

                  <div className="p-6 bg-slate-50 rounded-3xl border border-slate-200 text-[10px] font-bold text-slate-500 uppercase leading-relaxed">
                    <p className="mb-2 flex items-center gap-2">
                      <Cloud size={14} className="text-blue-500" />
                      Workspace: <span className="text-slate-900 font-black">{cloudConfig.workspaceId}</span>
                    </p>
                    <p className="flex items-center gap-2">
                      <Check size={14} className="text-blue-500" />
                      Persistence: <span className="text-slate-900 font-black">PostgreSQL / Supabase</span>
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="h-full flex flex-col animate-in fade-in duration-500">
              <div className="flex justify-between items-center mb-10">
                <div>
                  <h2 className="text-3xl font-black text-slate-950 uppercase tracking-tight flex items-center gap-3">
                    <History className="text-blue-600" /> Audit Logs
                  </h2>
                  <p className="text-xs text-slate-500 mt-2 font-bold uppercase tracking-widest">Workspace Action History</p>
                </div>
              </div>
              <div className="flex-1 overflow-y-auto space-y-4 pr-4 custom-scrollbar">
                {history.slice().reverse().map((log, i) => (
                  <div key={i} className="p-6 bg-slate-50 rounded-[24px] border border-slate-200 flex items-start gap-6 hover:bg-white hover:shadow-xl transition-all group">
                    <div className="w-12 h-12 rounded-2xl bg-white border border-slate-200 flex items-center justify-center shrink-0 shadow-sm group-hover:border-blue-200 transition-colors">
                      <Database size={20} className="text-slate-400 group-hover:text-blue-500 transition-colors" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">{new Date(log.timestamp).toLocaleString()}</span>
                        <span className="text-[9px] font-bold bg-slate-200 text-slate-600 px-2 py-0.5 rounded uppercase tracking-tighter">{log.type}</span>
                      </div>
                      <p className="text-sm font-bold text-slate-800 leading-snug">{log.message}</p>
                      {log.details && <p className="text-[11px] text-slate-500 mt-2 font-medium bg-white/50 p-3 rounded-xl border border-slate-100">{log.details}</p>}
                    </div>
                  </div>
                ))}
                {history.length === 0 && (
                  <div className="flex flex-col items-center justify-center h-full text-slate-300 gap-4 py-20">
                    ...
                    <span className="text-xs font-black uppercase tracking-[0.2em]">No actions recorded yet</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center h-full text-slate-400 gap-6 opacity-60">
              <BoxSelect size={80} strokeWidth={1} className="text-blue-500/30" />
              <div className="text-center">
                <span className="text-xs font-black uppercase tracking-[0.4em] block mb-2">Select a Registry Item</span>
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Choose a category from the sidebar to begin editing</span>
              </div>
            </div>
          )}
        </div>
