...
const InventoryPanel: React.FC<InventoryPanelProps> = ({
    ...
    const [showLinked, setShowLinked] = useState(true); // Default: show ALL PRO rows; toggle hides already-linked ones
    ...
    const handleDrop = (e: React.DragEvent, targetBimRow: any) => {
        ...
        try {
            ...
            // Use the active tab to determine type — the tab already ensures we're linking
            // racks to racks or equipment to equipment, so no need to compare row.type
            // (which can be undefined or stale in some data paths)
            const linkType = activeTab === 'RACKS' ? 'RACK' : 'EQUIPMENT';
            onLink(targetBimRow.id, data.id, linkType);
        } catch (err) { console.error('[InventoryPanel] Drop failed:', err); }
    };
    ...
    const rows = useMemo(() => {
        let data: any[] = [];
        // Calculate Linked IDs (BIM Items associated with a PRO record)
        // Method 1: Explicit proInventoryId field set during drag-link
        // Method 2: BIM item id matches a PRO item id (happens when link updated rack.id = proId)
        const proInventoryIds = new Set(proInventory.map(p => p.id));
        const linkedIds = new Set<string>();
        racks.forEach(r => {
            if ((r as any).proInventoryId) linkedIds.add((r as any).proInventoryId);
            // If the rack's id was changed to the PRO id during a previous link, detect it here
            if (proInventoryIds.has(r.id)) linkedIds.add(r.id);
        });
        equipment.forEach(e => {
            if (e.proInventoryId) linkedIds.add(e.proInventoryId);
            if (proInventoryIds.has(e.id)) linkedIds.add(e.id);
        });
        ...
        if (activeTab === 'RACKS') {
            const bimRacks = racks.map(r => ({
                ...
                source: 'BIM',
                type: 'RACK',
                isLinked: proInventoryIds.has(r.id) || !!((r as any).proInventoryId),
            ...
            // Lookup: proInventoryId → BIM rack (for change detection)
            const bimRackByProId = new Map<string, any>();
            bimRacks.forEach(r => { if (r.proInventoryId) bimRackByProId.set(r.proInventoryId, r); });
            // Also index by rack id directly (when rack.id === proId after link)
            bimRacks.forEach(r => { if (proInventoryIds.has(r.id)) bimRackByProId.set(r.id, r); });

            const proRacks = proInventory.filter(p => p.type === 'RACK' && matchesBuilding(p.building))
                // showLinked=true → show ALL; showLinked=false → hide rows that are already linked
                .filter(p => showLinked || !linkedIds.has(p.id))
                .map(p => {
                    const bimMatch = bimRackByProId.get(p.id);
                    const lineUpChanged = !!bimMatch && bimMatch.lineUp !== undefined && String(p.lineUp) !== String(bimMatch.lineUp);
                    const bayNoChanged = !!bimMatch && bimMatch.bayNo !== undefined && String(p.bayNo) !== String(bimMatch.bayNo);
                    return {
                        ...p,
                        source: 'PRO',
                        ...
                        lineUpChanged,
                        bayNoChanged,
                        sync: linkedIds.has(p.id) ? 'Verified' : 'Unknown',
            ...
            const bimEq = equipment.map(e => {
                ...
                return {
                    ...
                    source: 'BIM',
                    type: 'EQUIPMENT',
                    isLinked: proInventoryIds.has(e.id) || !!e.proInventoryId,
            ...
            // Lookup: proInventoryId → BIM equipment (for change detection)
            const bimEqByProId = new Map<string, any>();
            bimEq.forEach(e => { if (e.proInventoryId) bimEqByProId.set(e.proInventoryId, e); });
            bimEq.forEach(e => { if (proInventoryIds.has(e.id)) bimEqByProId.set(e.id, e); });

            const proEq = proInventory.filter(p => p.type === 'EQUIPMENT' && matchesBuilding(p.building))
                // showLinked=true → show ALL; showLinked=false → hide rows that are already linked
                .filter(p => showLinked || !linkedIds.has(p.id))
                .map(p => {
                    const bimMatch = bimEqByProId.get(p.id);
                    const lineUpChanged = !!bimMatch && bimMatch.lineUp !== undefined && String(p.lineUp) !== String(bimMatch.lineUp);
                    const bayNoChanged = !!bimMatch && bimMatch.bayNo !== undefined && String(p.bayNo) !== String(bimMatch.bayNo);
                    return {
                        ...p,
        ...
        let filteredData = data.filter(row => {
            return (Object.entries(filters) as [string, string][]).every(([key, value]) => {
                ...
                // Split on commas to support multi-value OR filtering (e.g. "001,003,005")
                const terms = value.split(',').map(t => t.trim().toLowerCase()).filter(Boolean);
                if (terms.length === 0) return true;

                const rowValue = String(row[key] || '').toLowerCase();

                // Handle specialized display mappings for status
                if (key === 'status') {
                    const statusLabel = (STATUS_LABELS[row.status as Status4D] || '').toLowerCase();
                    return terms.some(t => statusLabel.includes(t));
                }

                // OR match: row passes if it matches ANY of the comma-separated terms
                return terms.some(t => rowValue.includes(t));
            });
        });
        ...
        >
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 shrink-0">
                ...
                <div className="flex items-center gap-6 relative">
                    ...
                    >
                        <Link size={14} />
                        <span className="text-[10px] font-black uppercase tracking-widest">{!showLinked ? 'Show Linked' : 'Hide Linked'}</span>
                    </button>
                    ...
                    <div className="flex items-center gap-2 bg-blue-500/10 px-4 py-2 rounded-full border border-blue-500/20">
                        ...
                        <span className="text-[10px] font-black text-blue-400 uppercase">PRO-Sync Active</span>
                    </div>
                    <button
                        onClick={onProcess}
            ...
            <div className="flex-1 overflow-auto px-6 pb-6">
                <table className="w-full text-sm text-left border-collapse table-fixed">
                        ...
                        <tr className="text-[9px] font-black text-slate-500 uppercase tracking-[0.15em]">
                            ...
                            {activeTab === 'RACKS' ? (
                                <>
                                    ...
                                    <th className="px-6 py-4 w-40 cursor-pointer group" onClick={() => handleSort('lineUp')}>
                                    ...
                                    <th className="px-6 py-4 w-40 cursor-pointer group" onClick={() => handleSort('bayNo')}>
                    ...
                    <tbody className="divide-y divide-white/5">
                     ...
                     ${!isBim ? (row.isLinked ? 'bg-emerald-900/20' : 'bg-slate-900/40') : ''}
                  ...
                  `}
                                ...
                                >
                                    <td className="w-12 relative overflow-visible">
                                        {/* Adjacent connector lines - bonus visual when BIM+PRO happen to be next to each other */}
                                        {isLinkedToNext && (
                                            <div className="absolute left-1/2 bottom-0 w-0.5 h-1/2 bg-emerald-500/40 -translate-x-1/2 z-0" />
                                        )}
                                        {isLinkedToPrev && (
                                            <div className="absolute left-1/2 top-0 w-0.5 h-1/2 bg-emerald-500/40 -translate-x-1/2 z-0" />
                                        )}

                                        {/* Persistent link badge - always visible on any linked row */}
                                        {row.isLinked && (
                                            <div className="absolute inset-0 flex items-center justify-center z-10">
                                                <div
                                                    title={isBim ? 'Linked to PRO Inventory' : 'Linked to BIM'}
                                                    className={`p-1.5 rounded-full border shadow-lg ${isBim
                                                        ? 'bg-amber-500/10 border-amber-500/50 shadow-amber-500/20'
                                                        : 'bg-emerald-500/10 border-emerald-500/50 shadow-emerald-500/20'
                                                        }`}
                                                >
                                                    <Link2 size={10} className={`rotate-45 ${isBim ? 'text-amber-400' : 'text-emerald-400'
                                                        }`} />
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        <>
                                            <td className="px-6 py-4 font-mono text-slate-300 text-xs">
                                                {row.source === 'BIM' ? (
                                                    ...
                                                    <span className="bg-emerald-500/30 text-emerald-300 px-2 py-0.5 rounded font-bold flex items-center gap-1 w-fit" title="Linked to a BIM rack"><Link2 size={10} className="inline" />PRO</span>
                                                ) : (
                                            ...
                                            <td className={`px-6 py-4 text-slate-200 font-mono font-bold text-base text-center ${row.lineUpChanged ? 'text-amber-400 bg-amber-500/10' : ''}`}>
                                                {row.lineUp || '-'}
                                            </td>
                                            <td className={`px-6 py-4 text-slate-200 font-mono font-bold text-base text-center ${row.bayNoChanged ? 'text-amber-400 bg-amber-500/10' : ''}`}>
                                                {row.bayNo || '-'}
                                            </td>
                                        ...
                                        <>
                                            <td className="px-6 py-4 font-mono text-slate-300 text-xs">
                                                {row.source === 'BIM' ? (
                                                    ...
                                                    <span className="bg-emerald-500/30 text-emerald-300 px-2 py-0.5 rounded font-bold flex items-center gap-1 w-fit" title="Linked to a BIM item"><Link2 size={10} className="inline" />PRO</span>
                                                ) : (
                                                    <span className="bg-emerald-500/20 text-emerald-300 px-2 py-0.5 rounded font-bold">PRO</span>
                                                )}
                                            </td>
