...
async function restoreAdmin() {
    console.log('--- FINAL PROD ADMIN RESTORATION ---');

    // 1. Load harvested data
    const harvested = JSON.parse(fs.readFileSync('master_admin_registry.json', 'utf8'));
    ...
    // 3. Inject Solid Registries
    console.log(`Injecting ${harvested.vendors.length} Vendors and ${harvested.equipmentModels.length} Equipment Models...`);

    wsData.vendors = harvested.vendors;
    wsData.equipmentDefs = harvested.equipmentModels;

    // 4. Restore Building & Rack Standard Defs
    wsData.buildingDefs = [
    ...
    // 5. Restore Owners & Statuses (Verified from Feb 22 source)
    wsData.owners = [
    ...
    if (updateError) {
        ...
        console.log('SUCCESS: Production Admin Area fully restored with solid records.');
        console.log('Placeholders "BBBB" and "Generic" have been replaced with real library entries.');
    }
}
