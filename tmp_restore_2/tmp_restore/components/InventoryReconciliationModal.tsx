...
export const InventoryReconciliationModal: React.FC<InventoryReconciliationModalProps> = ({
    ...
    const diff = useMemo(() => {
        ...
        racks.forEach(r => {
            if (!r.proInventoryId) {
                ...
                if (pro) {
                    // Check for Updates (e.g. Location mismatch?)
                    // Current instructions emphasize RMU update for equipment.
                    // For Racks, maybe checking specific fields? For now assume valid link implies no update unless logic added.
                }
            }
        });
        ...
        // Process Deletes (Pro items not linked to any BIM Item)
    ...
    const handleDownload = (format: 'json' | 'txt') => {
        ...
        let content = '';
        let filename = `inventory-instruction-${date}`;

        if (format === 'json') {
            const payload = {
                metadata: { date, generatedBy: "Lumen Asset LCM" },
                operations: {
                    create: diff.creates.map(c => ({
                        type: c.type,
                        tempId: c.item.id, // Using BIM ID as Temp ID
            ...
            content = `INVENTORY RECONCILIATION INSTRUCTION\nDate: ${date}\n\n`;
            content += `=== CREATE (${diff.creates.length}) ===\n`;
            content += `These items exist in Reality (BIM) but not in Granite.\n`;
            diff.creates.forEach(c => {
                content += `[${c.type}] ID:${c.item.id} Name:${c.type === 'RACK' ? (c.item as Rack).label : c.item.name}\n`;
            });
            console.log(content);

            content += `\n=== UPDATE (${diff.updates.length}) ===\n`;
            content += `These items have attribute mismatches.\n`;
            diff.updates.forEach(u => {
                content += `[${u.bim.name || 'Item'}] (ID: ${u.pro.id}) -> ${u.changes.join(', ')}\n`;
            });

            content += `\n=== DELETE (${diff.deletes.length}) ===\n`;
            content += `These items are in Granite but NOT in Reality (BIM).\n`;
            diff.deletes.forEach(d => {
                content += `[${d.type}] ${d.id} (${d.name}) @ ${d.location}\n`;
            });
            filename += '.txt';
        }
            ...
            <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-5xl h-[80vh] flex flex-col shadow-2xl">
                ...
                <div className="bg-slate-950 px-8 py-6 border-t border-white/10 rounded-b-2xl flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        >
                            <FileText size={16} /> Download Report (TXT)
                        </button>
                        <button
                        ...
                        >
                            <FileJson size={16} /> Generate Instructions (JSON)
                        </button>
                    </div>
                </div>
