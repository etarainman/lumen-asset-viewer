...
import {
  ...
  Type, List, Maximize, Maximize2, Map as MapIcon, SlidersHorizontal, ChevronUp, ChevronDown, Check, X, Plus, Cloud, Loader2, Hash
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const [showRMULabels, setShowRMULabels] = useState(true);
  ...
  const alerts = useMemo<Alert[]>(() => {
    try {
      ...
      // 1. Check for Duplicate Addresses (Suite + LineUp + BayNo) per Building
      buildings.forEach(b => {
        const racksByLoc = new Map<string, Rack[]>();

        (b.racks || []).forEach(r => {
          // Key: Suite.LineUp.BayNo
          // We default to 'UNK' if missing to avoid collision with '000' which might be valid
          const s = r.suite || 'UNK';
          ...
          if (racksInLoc.length > 1) {
            ...
            racksInLoc.forEach(r => {
              list.push({
                id: `DUP-${r.id}`,
                severity: 'CRITICAL',
                message: 'Duplicate Rack Address',
                details: `Multiple racks claim position ${key} in ${b.label}.`,
                sourceId: r.id,
                sourceType: 'RACK',
                location: `${b.label} / ${key}`,
                buildingId: b.id,
                siteId: b.siteId
              });
            });
        ...
        // 2. Check for Stakeholder Mismatches (Equipment Owner != Rack Owner)
        (b.equipment || []).forEach(e => {
          ...
          if (parentRack && e.ownerId && parentRack.ownerId && e.ownerId !== parentRack.ownerId) {
            ...
            list.push({
              id: `OWNER-MISMATCH-${e.id}`,
              severity: 'WARNING',
              message: 'Stakeholder Mismatch',
              details: `Equipment belongs to ${eqOwnerName} but is installed in a ${rackOwnerName} rack.`,
              sourceId: e.id,
              sourceType: 'EQUIPMENT',
              location: `${b.label} / ${parentRack.label} / RMU ${e.baseRMU}`,
              buildingId: b.id,
              siteId: b.siteId
            });
          }
      ...
      console.error("Alerts calculation failed", e);
      return [];
    }
  }, [buildings, owners]);
    ...
    <div className="flex flex-col h-screen w-screen bg-slate-950 text-slate-200 overflow-hidden font-sans">
        ...
        <header className="h-16 glass-panel border-b border-white/5 flex items-center justify-between px-8 z-[100]">
          ...
          <div className="flex items-center gap-3">
            {isDirty && (
              ...
              >
                {cloudStatus === 'CONNECTING' ? <Loader2 size={18} className="animate-spin" /> : <Save size={18} />}
                <span className="text-[10px] font-black uppercase tracking-widest">
                  {cloudStatus === 'CONNECTING' ? 'Saving...' : 'Save'}
                </span>
              </button>
            )}
              ...
              {(() => {
                ...
                return (
                  <EditRackCard rack={rack} rackDefs={rackDefs} equipmentDefs={equipmentDefs} owners={owners} statuses={statuses} buildingLabel={activeBuilding.label} otherRacks={activeBuilding.racks.filter(r => r.id !== selectedRackId)}
                    ...
                    onMove={(dir, shiftKey) => {
                      // Fine nudge: 0.25ft (3 inches), Shift+nudge: 1.0ft (12 inches)
                      const step = shiftKey ? 1.0 : 0.25;
        ...
        {showInventory && (
          <InventoryPanel
            ...
            equipment={(activeBuilding ? [activeBuilding] : siteBuildings).flatMap(b => b.equipment.map(e => ({
              ...e,
              building: b.label,
              vendorName: vendors.find(v => v.id === e.vendorId)?.name || '-',
              ownerName: owners.find(o => o.id === e.ownerId)?.name || '-',
              model: equipmentDefs.find(d => d.id === e.definitionId)?.name || '-'
            })))}
            ...
            filterBuildingCode={activeBuilding?.label}
      ...
      {showReconciliation && (
        <InventoryReconciliationModal
          ...
          proInventory={proInventory}
          siteId={activeSite?.name || 'SITE'} // Use name (e.g. CSRKCOCF) or ID? sites[0].id usually. 
        // activeSite.id for code compatibility.
        // User said "e.g. CSRKCOCF". Site definition has name/id.
        // I'll use activeSite?.id as the Code.
        />
      )}
    </div>
  );
