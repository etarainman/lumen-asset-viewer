...
const InventoryPanel: React.FC<InventoryPanelProps> = ({
    ...
    const processCsvFile = (text: string) => {
        ...
        const possibleEqHeaders = ['EQPT_ID', 'EQPT ID', 'EQUIPMENT ID', 'ASSET ID', 'ASSET_TAG', 'DEVICE ID'];
        ...
        if (isGranite) {
            ...
            for (const ph of possibleEqHeaders) {
                const found = headers.findIndex(h => h.includes(ph));
            ...
            lines.slice(1).forEach((line, idx) => {
                ...
                if (hasRack) {
                    ...
                    // We mark the row as captured because it contained a valid rack, 
                    // even if we skipped adding it to valid list due to duplication.
                    rowCaptured = true;
                }
                ...
                if (hasEq) {
                    ...
                    const cleanName = eqName.split('.')[1] || eqName || 'Unknown Eq';
                ...
                if (!rowCaptured) {
                    equipmentList.push({
                        id: `ROW-${rowNum}-MISSING`,
                        name: '[MISSING IDs]',
                        type: 'EQUIPMENT', // Default to equipment so it appears in list
                        rackId: hasRack ? rackId : '',
                        proInventoryId: '',
                        status: Status4D.PROPOSED,
                        location: bayName,
                        lineUp: lineUp !== '000' ? lineUp : '-',
                        bayNo: bayNo !== '000' ? bayNo : '-',
                        rmu: '0',
                        building: building || '-',
                        suite,
                        model: 'Unidentified Row',
                        vendorName: 'Unknown',
                        ownerName,
                        csvRowNumber: rowNum,
                        usage: 'Invalid Row Data'
                    });
                }
            });
            ...
            items = lines.slice(1).map((line, idx) => {
                ...
                return {
                    id, name, type, rackId: '', status: Status4D.RETAIN, location: loc, lineUp: '000', bayNo: '000', rmu: '0',
                    ownerName: 'Lumen', csvRowNumber: idx + 2
                } as CsvPreviewItem;
            });
        }
