...
import {
  ...
  Type, List, Maximize, Maximize2, Map, SlidersHorizontal, ChevronUp, ChevronDown, Check, X, Plus, Cloud, Loader2, Hash
} from 'lucide-react';
...
const App: React.FC = () => {
    ...
    setBuildings(prev => prev.map(b => {
      ...
      if (up.racks || up.suites) {
        ...
        const reconciledRacks = updatedBuilding.racks.map(r => {
          // 1. Detect Suite
          let detectedSuiteName = "000";
          const foundSuite = suites.find(s =>
            r.x >= s.x && r.x <= (s.x + s.width) &&
            r.y >= s.y && r.y <= (s.y + s.depth)
          );
          ...
          // Update if changed
          if (r.suite !== detectedSuiteName || r.location !== newLocation) {
            return { ...r, suite: detectedSuiteName, location: newLocation };
          }
          return r;
        });
  ...
  return (
    <div className="flex flex-col h-screen w-screen bg-slate-950 text-slate-200 overflow-hidden font-sans">
      ...
      <main className="flex-1 relative bg-slate-900 overflow-hidden">
                ...
                return (
                  <EditRackCard rack={rack} rackDefs={rackDefs} equipmentDefs={equipmentDefs} owners={owners} statuses={statuses}
                    ...
                    onArrayClone={(seedRack, count, spacing, direction) => {
                      ...
                      // Parse lineup and bay from label (e.g., "001.002" -> lineup: "001", bay: 2)
                      const labelParts = seedRack.label.split('.');
                      const lineup = labelParts[0] || '001';
                      let currentBay = parseInt(labelParts[1] || '1');
                      ...
                      for (let i = 0; i < count; i++) {
                        currentBay++;
                        const bayStr = currentBay.toString().padStart(3, '0');
                      ...
                      handleUpdateBuilding(activeBuilding.id, { racks: newRacks });
                      // addToHistory(`Created ${count} rack clone(s) from ${seedRack.label}`);
                    }}
                  />
                );
              })()}
        ...
        {showInventory && (
          <InventoryPanel
            ...
            equipment={siteBuildings.flatMap(b => b.equipment)}
            racks={siteBuildings.flatMap(b => b.racks)}
            proInventory={proInventory}
            filterBuildingCode={null}
      ...
      />
      {showReconciliation && activeBuilding && (
