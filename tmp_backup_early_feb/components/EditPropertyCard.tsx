...
import { X, GripHorizontal, Trash2, Camera, FileText, HardDrive, User, Layers, Box, AlertTriangle } from 'lucide-react';
...
const EditPropertyCard: React.FC<EditPropertyCardProps> = ({
  item, equipmentDefs, racks, vendors, owners, statuses, onUpdate, onClose, mode, onDelete, clashingIds
}) => {
  ...
  // Sync selected vendor with current definition
  useEffect(() => {
    if (item.definitionId) {
      const def = equipmentDefs.find(d => d.id === item.definitionId);
      if (def) setSelectedVendorName(def.manufacturer || 'Generic');
    }
  }, [item.definitionId, equipmentDefs]);
  ...
  return (
    <>
      <div className="absolute w-[280px] bg-white shadow-2xl rounded-3xl border border-slate-400 z-[120] animate-in fade-in zoom-in duration-300 flex flex-col overflow-hidden" style={{ left: `${position.x}px`, top: `${position.y}px` }}>
        ...
        {clashingIds?.has(item.id) && (
          <div className="bg-red-500/10 border-l-4 border-red-500 p-3 mx-4 mt-4 flex items-start gap-2">
            <AlertTriangle size={16} className="text-red-500 shrink-0 mt-0.5" />
            <div>
              <p className="text-[10px] font-black text-red-500 uppercase tracking-wider">Collision Detected</p>
              <p className="text-[9px] text-red-400 leading-tight">This equipment overlaps with another item in the rack.</p>
            </div>
          </div>
        )}
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="space-y-3">
            ...
            {/* 2-Step Equipment Selection */}
            <div>
              <label className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1 block">Vendor</label>
              <select
                value={selectedVendorName}
                onChange={e => setSelectedVendorName(e.target.value)}
                disabled={isReadOnly}
                className="w-full text-[10px] font-bold border-b border-slate-300 pb-1 outline-none bg-transparent uppercase text-slate-950"
              >
            ...
            <div>
              ...
              <select
                ...
                onChange={e => onUpdate(item.id, { definitionId: e.target.value })}
                disabled={isReadOnly || !selectedVendorName}
                className="w-full text-[10px] font-bold border-b border-slate-300 pb-1 outline-none bg-transparent uppercase truncate text-slate-950"
              >
                <option value="">Select Model...</option>
                {equipmentDefs
                  .filter(d => (d.manufacturer || 'Generic') === selectedVendorName)
