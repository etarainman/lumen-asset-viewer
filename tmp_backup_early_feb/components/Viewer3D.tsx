...
const Viewer3D = forwardRef<Viewer3DHandle, Viewer3DProps>(({
  ...
  rackDefs, equipmentDefs = [], showRackLabels = true, showRMULabels = true, showEqLabels = true, showRoof = true, showGrid = true, showSuites = true, visibleStatuses = Object.values(Status4D), onSelectRack, onSelectEquipment, focusTrigger = 0, clashingIds, hiddenLineUps
}, ref) => {
        ...
        b.racks.forEach(rack => {
          ...
          rackEquipment.forEach(eq => {
            ...
            if (showEqLabels) {
              ...
              labelDiv.className = 'text-[9px] font-black uppercase tracking-tighter text-white/90 bg-black/60 px-1 py-0.5 rounded border border-white/10 pointer-events-none whitespace-nowrap text-center flex flex-col items-center justify-center min-w-[40px] shadow-lg';
              labelDiv.innerHTML = `<span class="opacity-60 text-[7px] leading-tight">${vendor}</span><span class="leading-tight">${model}</span>`;
              const labelObj = new CSS2DObject(labelDiv);
  ...
  }, [buildings, buildingDefs, activeBuildingId, viewLevel, shellOpacity, statuses, owners, colorMode, colorCodingEnabled, rackDefs, equipmentDefs, showRackLabels, showRMULabels, showRoof, showSuites, visibleStatuses, selectedRackId, selectedEquipmentId, clashingIds, hiddenLineUps]);
